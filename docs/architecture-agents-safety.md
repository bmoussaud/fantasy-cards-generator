...
|-------|------------------------|-------------|
| `pre_prompt` | Stops before any card/audit reservation; `ProblemDetails` raised | `422 prompt_rejected` |
| `post_text` | Stops before any card/audit reservation; `ProblemDetails` raised | `422 generated_text_rejected` |
| `post_art_prompt` | Stops before any card/audit reservation; `ProblemDetails` raised | `422 generated_art_rejected` |
| `post_image` | Generated bytes discarded before any card/audit reservation; `ProblemDetails` raised | `422 generated_art_rejected` |

All four stages use the same explicit deny-wins refusal path. A BLOCK stops
the pipeline after that decision. The bounded HTTP status and public code may
be returned and counted in allowlisted telemetry, but owner/request/idempotency
hashes, titles/details/headers, rejected content, and error payloads are not
written to application repositories.

Simultaneous duplicates handled by the same application instance share a
request-local single-flight result, including the same bounded refusal, without
writing coordination metadata. Only after all applicable moderation allows
does the service acquire the durable card reservation immediately before
persisting a safe completed or technical-failure partial card. This removes
Cosmos deletion from the refusal correctness path.

The same rule applies to artwork retry: post-image refusal creates no retry
audit and leaves the pre-existing safe `awaiting_artwork_retry` card unchanged.
A retry audit reservation is created only after the new image passes
moderation.

### Moderation decisions stored in Cosmos

Every moderation stage that is actually reached appends its
`ModerationDecision` only to the request-local `moderation` list. Completed
cards and technical-failure partial cards persist decisions only after all
applicable moderation stages allow.

A denial leaves no idempotency or generation-audit repository record, so a
later request using the same idempotency key is evaluated as a new request.
Concurrent duplicates on different application replicas can therefore repeat
upstream work until one reaches the post-moderation durable reservation; the
deterministic card ID and atomic repository reservation still prevent distinct
persisted card results. This is the deliberate availability/cost tradeoff
required to guarantee that refusal correctness never depends on cleanup.
Legacy `audit_failed` moderation records can still be read by the compatibility
replay path until their TTL expires, but new refusals never create them.
...
|------|------------------------|--------------------------|
| **Authoritative deny wins** | Heuristic text/image denials and hosted-agent refusals abort without card, blob, or refusal-audit persistence; saved-photo Content Safety denials reject the save. | Any authoritative deny — including a managed guardrail denial — must stop the protected path. No later allow may reverse it. |
...
| Heuristic BLOCK (`post_image`) | Content denial on artwork | `HeuristicModerationService` | `422 generated_art_rejected`; no persistence |
...
| Successful image/edit followed by post-image heuristic BLOCK | Content denial on artwork | `HeuristicModerationService` | `422 generated_art_rejected`; no persistence |
...
Once either image path has produced image bytes, the post-image heuristic
moderation stage behaves the same on both paths: a BLOCK discards the bytes,
stops before a card or retry-audit reservation, persists no new
card/blob/refusal metadata, and returns `422 generated_art_rejected`.
`awaiting_artwork_retry` remains reserved for technical image failure or timeout
after safe text, not policy refusal; a refusal during retry preserves that
pre-existing safe partial card.
...
refusal returns `422` through the no-persistence refusal path. None invokes a
direct or weaker card-text generator.
...
3. **Post-image moderation after generation/edit.** If the image generation or
   ...
   checks on the generated output. A BLOCK there discards the bytes and leaves
   no card, blob, or generation-audit record.
