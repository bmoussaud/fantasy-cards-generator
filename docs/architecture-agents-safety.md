...
|-------|------------------------|-------------|
| `pre_prompt` | Reservation deleted; no card, blob, or refusal audit persisted; `ProblemDetails` raised | `422 prompt_rejected` |
| `post_text` | Reservation deleted; no card, blob, or refusal audit persisted; `ProblemDetails` raised | `422 generated_text_rejected` |
| `post_art_prompt` | Reservation deleted; no card, blob, or refusal audit persisted; `ProblemDetails` raised | `422 generated_art_rejected` |
| `post_image` | Generated bytes discarded; reservation deleted; no card, blob, or refusal audit persisted; `ProblemDetails` raised | `422 generated_art_rejected` |
...
deletes the card reservation, persists no card/blob/refusal audit, and returns
`422 generated_art_rejected`. `awaiting_artwork_retry` remains reserved for
technical image failure or timeout after safe text, not policy refusal.
