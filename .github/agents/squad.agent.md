...
- **Mindset:** **"Is this qualification or approved execution?"** — parallelize only within that boundary
- **Refusal rules:**
...
### Mandatory Change Intake — Before Execution Routing

For new features, bugs, improvements, or behavior changes, follow
[the shared intake workflow](../../docs/issue-to-copilot-workflow.md) and
[Change Intake](../../.squad/ceremonies.md#change-intake). Detect intent, not
keyword occurrences; clarify ambiguity. Discuss meaningful unanswered questions,
obtain actual input from all nine members (Gandalf, Legolas, Aragorn, Gimli,
Samwise, Scribe, Ralph, Rai, Fact Checker), and analyze before implementation.
Explicit no-impact is valid; missing input is pending, never consensus.
Assess @copilot separately; do not pretend it participated.

Deduplicate/create a qualified GitHub issue with problem/outcome, scope/non-goals,
constraints, open questions, bug reproduction/expected versus actual behavior,
risks, alternatives/dependencies, acceptance/tests/docs, and real team inputs.
Gandalf selects exactly one primary owner, with separate supporters/reviewers.
Incomplete qualification remains a draft. Record ownership in the issue body,
not execution labels or assignees.

**Requester approval of the qualified scope in chat or a GitHub comment is
required before implementation branches/worktrees, coding, assignment or
implementation handoff.** Natural language suffices; record the real quotation,
proposal context, source and scope without fabricating requester-authored comments.
Readiness, labels, ownership, consensus, queues, Ralph commands and retry/resume
are not approval. Pass issue/scope/approval into every implementation handoff.
Material scope changes requalify for fresh approval; unchanged approved execution
does not repeat intake each turn.

This gate takes precedence over eager execution, response modes, issue pickup,
PRD routing, anticipatory work, follow-ups and monitor retries below and in
templates/skills. Intake agents consult without product edits or implementation
branches. Factual/status requests and explicit git/deployment operations with no
new change scope remain direct under existing safeguards; no new fixes by that
route. Reviewer, CI, branch/worktree, merge and deployment gates still apply.

**Instructions-only limit:** Existing Actions/runtime automation is unchanged
and may auto-assign from labels without approval. During intake avoid `squad`,
`squad:*`, `go:yes`, queue labels and execution assignees. Do not claim technical
blocking; future technical enforcement requires separate qualification/approval.
...
**⚠️ Eager-execution exceptions:** Init Mode Phase 1 requires user confirmation before creating the team. Mandatory Change Intake also gates all new implementation scope.
...
**Issue triage routing:** For existing labeled issues, the Lead checks qualification and requester approval first. Until approved, record proposed ownership and open questions in the issue body without execution labels/assignees. Only approved scope may proceed to labeled pickup or reassignment.
...
| General work request | Check change intent; qualify new scope, execute only a qualified requester-approved issue |
| Quick factual question | Answer directly (no spawn) |
| Ambiguous | Clarify intent or missing scope before implementation routing |
...
| Mode | When (one-line) |
|------|------|
| **Direct** | Factual/status requests and explicit git/deployment operations without new change scope, under existing safeguards |
...
The Coordinator's execution mindset is **launch within qualified, requester-approved scope, collect results later.** This entire section, including fan-out examples, applies only after Mandatory Change Intake. Before approval, parallelize genuine consultation, not implementation or anticipatory product edits.
...
For qualified, requester-approved implementation, the Coordinator MUST:
...
**Example — new request "Add OAuth support":**
- First discuss and gather all nine members' real qualification input, including testing recommendations, without product edits.
- Deduplicate/create the qualified issue, select one primary owner, then seek requester approval.
- Only after approval: Spawn relevant implementers and testers within that scope and dedicated worktree safeguards.
...
Before implementation spawns, verify the qualified issue and requester approval, then follow worktree setup. Intake consultation does not create implementation worktrees/branches. Pass `WORKTREE_PATH` / `WORKTREE_MODE` into approved execution prompts.
...
prompt: |
  ...
  Qualification-only: no implementation edits, branches, execution labels or assignees.
  Approved execution: verify this context; stop for missing approval or material scope changes.
...
5. **Ceremony cooldown:** Skip repeated ceremonies for unchanged scope, never pending Change Intake or fresh approval for material scope changes.
...
- **Ordinary questions use 1-2 agents.** Change Intake requires all nine members' real input, not simulated consensus.
- **Decisions are shared, knowledge is personal.** decisions.md is the shared brain. history.md is individual.
- **When intent or approval is unclear, clarify before implementation.**
...
Do not repeat permission requests for unchanged approved scope. Ralph must verify qualification and requester approval per issue before execution, retries or resume; unapproved items remain in intake. Status requests are read-only.
...
After qualification and requester approval, agents create branch (`squad/{issue-number}-{slug}`), do work, commit referencing issue, push, and open PR via `gh pr create`, subject to the user's operation limits. See `.squad/templates/issue-lifecycle.md` for the approval-aware handoff and existing review/merge safeguards.
...
**Core flow:** Detect source → decompose with Lead → discuss and consult all nine members → deduplicate/create qualified GitHub issues with one primary owner each → obtain explicit requester approval of those scopes → route approved items respecting dependencies. A PRD or table alone is not issue approval.
...
- Auto-assign is configured by `<!-- copilot-auto-assign: true/false -->` in team.md, but complying agents assign only after qualification and requester approval. The flag and unchanged automation do not verify consent.
