...
📌 Recovered note (2026-09-07T08:52:21.506+00:00): PR #108 resolved issue #97 in `docs/architecture-agents-foundry.md`: the proposed hosted-agent text hop has an 8.15s budget before falling back to the legacy text path, preserving the normal image budget and `awaiting_artwork_retry` behavior. This is an architecture plan, not an implemented runtime change.
📌 Epic created (2026-09-09T12:10:02.694+00:00): Issue #125 opened to drive Foundry card-orchestrator integration into `app/generation.py` with versioned contract, feature flag, deterministic safety, existing artwork workflow, durable dev deployment, observability/evals/security/rollback, and progressive activation. Out of scope: auth/persistence/image migration into agent, broad MCP toolbox, premature prompt agents. — Gandalf
📌 Epic #125 scope & review (2026-09-09T12:25:19Z): Analyzed coordinator request to create Foundry integration issue. Found #125 already created; refined scope, identified overlap with #109/#122/#123 and placeholder #102. After Samwise's REJECTED review (RBAC description error) and Aragorn's lockout, independently identified and corrected the RBAC blocker: two assignments conditional on `enableFoundryAgentAccess` at project scope; Cognitive Services User unconditional at account scope. Samwise re-reviewed and APPROVED. Issue #125 now ready for implementation.
